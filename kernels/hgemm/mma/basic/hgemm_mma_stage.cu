...
#define CP_ASYNC_BULK(dst, src, bytes)                                         \
  asm volatile(                                                                \
      ...
      "%2;\n" ::"l"(dst),                                                      \
      "r"(src), "n"(bytes))
#define CP_ASYNC_BULK_TENSOR_2D(tensorMap, coord_i, coord_j, src)         \
  asm volatile(                                                                \
...
__constant__ CUtensorMap tma_map_c;
...
template <const int MMA_M = 16, const int MMA_N = 8, const int MMA_K = 16,
          ...
          const int WARP_TILE_M = 4, const int WARP_TILE_N = 4,
          const int A_PAD = 0, const int B_PAD = 0, const int K_STAGE = 2,
          const bool BLOCK_SWIZZLE = true, const bool COLLECTIVE_STORE = true>
__global__ void __launch_bounds__(256)
...
#pragma unroll
  ...
  if (COLLECTIVE_STORE && !BLOCK_SWIZZLE) {
...
#pragma unroll
      for (int j = 0; j < WARP_TILE_N; ++j) {
        ...
        // 改成cp async bulk tensor之后tflops下降了非常多，直接优化效果极不理想。
        // 改进后memory排布按照每个i、j的warp连续而非warp内连续外加stage的形式可能会好很多，但是还没调通。
        if (lane_id == 0) {
          uint32_t smem_c_ptr = __cvta_generic_to_shared(&s_c[warp_m][warp_n]);
          CP_ASYNC_BULK_TENSOR_2D(&tma_map_c, store_lane_gmem_c_n, store_lane_gmem_c_m, smem_c_ptr);
          // CP_ASYNC_BULK(&C[store_gmem_c_addr], smem_c_ptr, MMA_M * MMA_N * sizeof(half));
...
template <const int MMA_M = 16, const int MMA_N = 8, const int MMA_K = 16, 
          ...
          const bool BLOCK_SWIZZLE = true, const bool COLLECTIVE_STORE = true>
__global__ void __launch_bounds__(256)
    hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_cpbulk_kernel(half *A, half *B, 
                                                          half *C, int M, int N, 
                                                          int K) {
  const int bx = ((int)BLOCK_SWIZZLE) * blockIdx.z * gridDim.x + blockIdx.x;
  const int by = blockIdx.y;
...
#pragma unroll
  for (int k = 0; k < (K_STAGE - 1); ++k) {
    ...
    uint32_t load_smem_a_ptr = smem_a_base_ptr + (s_a_stage_offset * k + load_smem_a_m * (BK + A_PAD) + load_smem_a_k) * sizeof(half);
    uint32_t load_smem_b_ptr = smem_b_base_ptr + (s_b_stage_offset * k + load_smem_b_k * (BN + B_PAD) + load_smem_b_n) * sizeof(half);
...
#pragma unroll
    for (int i = 0; i < WARP_TILE_M; ++i) {
      int warp_smem_a_m = i * (MMA_M * MMA_TILE_M) + warp_m * MMA_M;
      int lane_smem_a_m = warp_smem_a_m + lane_id % 16; // 0~15
      int lane_smem_a_k = (lane_id / 16) * 8; // 0,8
...
#pragma unroll
    for (int j = 0; j < WARP_TILE_N; ++j) {
      int warp_smem_b_n = j * (MMA_N * MMA_TILE_N) + warp_n * MMA_N;
...
#if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900)
  if (COLLECTIVE_STORE) {
    __shared__ __align__(128) half s_c[1][MMA_TILE_M][MMA_TILE_N][MMA_M][MMA_N];
    // WARP_TILE_M == 4, WARP_TILE_N == 4
    // MMA_M == 16, MMA_N == 8
    // MMA_TILE_M == 2, MMA_TILE_N == 4
#pragma unroll
    for (int i = 0; i < WARP_TILE_M; ++i) {
#pragma unroll
      for (int j = 0; j < WARP_TILE_N; ++j) {
        int stage_sel = ((i * WARP_TILE_N) + j) % 2;  // 0, 1
        stage_sel = 0;
        RC[i][j][0] = 0x3C004000;
        RC[i][j][1] = 0x42004400;
        uint32_t smem_c_base_ptr = __cvta_generic_to_shared(&s_c[stage_sel][warp_m][warp_n]);
        uint32_t lane_smem_c_ptr = (smem_c_base_ptr + (lane_id % 16) * MMA_N * sizeof(half));
        STMATRIX_X2(lane_smem_c_ptr, RC[i][j][0], RC[i][j][1]);

        // if (lane_id < 16) {
        //   int store_warp_gmem_c_m = i * (MMA_M * MMA_TILE_M) + warp_m * MMA_M;
        //   int store_warp_gmem_c_n = j * (MMA_N * MMA_TILE_N) + warp_n * MMA_N;
        //   int store_lane_gmem_c_m = by * BM + store_warp_gmem_c_m;  // 1 * 128 + 3 * 16 * 2 + 1 * 16 + 1~15
        //   int store_lane_gmem_c_n = bx * BN + store_warp_gmem_c_n;
          
        //   int store_gmem_c_addr = (store_lane_gmem_c_m + lane_id) * N + store_lane_gmem_c_n;
        //   printf("bx: %d, by: %d, lane id: %d, warp_m: %d, warp_n: %d, %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f, global row: %d global col: %d\n", bx, by, lane_id, warp_m, warp_n,
        //     __half2float(s_c[stage_sel][warp_m][warp_n][lane_id][0]), __half2float(s_c[stage_sel][warp_m][warp_n][lane_id][1]), 
        //     __half2float(s_c[stage_sel][warp_m][warp_n][lane_id][2]), __half2float(s_c[stage_sel][warp_m][warp_n][lane_id][3]), 
        //     __half2float(s_c[stage_sel][warp_m][warp_n][lane_id][4]), __half2float(s_c[stage_sel][warp_m][warp_n][lane_id][5]), 
        //     __half2float(s_c[stage_sel][warp_m][warp_n][lane_id][6]), __half2float(s_c[stage_sel][warp_m][warp_n][lane_id][7]), (store_lane_gmem_c_m + lane_id), store_lane_gmem_c_n);
        //   LDST128BITS(C[store_gmem_c_addr]) = LDST128BITS(s_c[warp_m][warp_n][lane_id][0]);
        // }
        // __syncthreads();

        __syncthreads();
        if (tid == 0) {
          uint32_t smem_c_ptr = __cvta_generic_to_shared(&s_c[stage_sel]);
          CP_ASYNC_BULK_TENSOR_2D(&tma_map_c, bx * BN + (MMA_N * MMA_TILE_N) * j, by * BM + (MMA_M * MMA_TILE_M) * i, smem_c_ptr);
          CP_ASYNC_BULK_COMMIT_GROUP();
        }
        CP_ASYNC_BULK_WAIT_GROUP(0);
        __syncthreads();
...
#pragma unroll
      for (int j = 0; j < WARP_TILE_N; ++j) {
        int store_warp_smem_c_m = i * (MMA_M * MMA_TILE_M) + warp_m * MMA_M;
        int store_warp_smem_c_n = j * (MMA_N * MMA_TILE_N) + warp_n * MMA_N;
        int store_lane_gmem_c_m = by * BM + store_warp_smem_c_m + lane_id / 4;
...
#define LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4_DSMEM_KERNEL(stages)      \
  {                                                                            \
    ...
    cuuint32_t box_dim[] = {MMA_N, MMA_M};                                     \
    cuuint32_t element_strides[] = {1u, 1u};                                   \
    cuTensorMapEncodeTiled(&tma_desc, CU_TENSOR_MAP_DATA_TYPE_FLOAT16, 2,      \
        c.data_ptr(), global_dim, global_strides, box_dim, element_strides,    \
        CU_TENSOR_MAP_INTERLEAVE_NONE, CU_TENSOR_MAP_SWIZZLE_NONE,             \
        CU_TENSOR_MAP_L2_PROMOTION_NONE, CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);   \
    cudaMemcpyToSymbol(tma_map_c, &tma_desc, sizeof(CUtensorMap));             \
    dim3 block(NUM_THREADS);                                                   \
    dim3 grid(div_ceil(N, BN), div_ceil(M, BM));                               \
    hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_kernel<                     \
        MMA_M, MMA_N, MMA_K, MMA_TILE_M, MMA_TILE_N, WARP_TILE_M, WARP_TILE_N, \
...
// #if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900)
// // 128x128, mma2x4, warp4x4(64,32,16), stages, block swizzle, dsmem
// #define LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(stages, stride)\
//   {                                                                            \
//     const int smem_max_size = ((stages) * BM * (BK + A_PAD) * sizeof(half) +   \
//                                (stages) * BK * (BN + B_PAD) * sizeof(half));   \
//     cudaFuncSetAttribute(                                                      \
//         hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_cpbulk_kernel<          \
//             MMA_M, MMA_N, MMA_K, MMA_TILE_M, MMA_TILE_N, WARP_TILE_M,          \
//             WARP_TILE_N, A_PAD, B_PAD, (stages), true, true>,                  \
//         cudaFuncAttributeMaxDynamicSharedMemorySize, 98304);                   \
//     const int N_SWIZZLE = (N + (stride) - 1) / (stride);                       \
//     dim3 block(NUM_THREADS);                                                   \
//     dim3 grid((div_ceil(N, BN) + N_SWIZZLE - 1) / N_SWIZZLE, div_ceil(M, BM),  \
...
//     hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_cpbulk_kernel<              \
//         MMA_M, MMA_N, MMA_K, MMA_TILE_M, MMA_TILE_N, WARP_TILE_M, WARP_TILE_N, \
//         A_PAD, B_PAD, (stages), true, true><<<grid, block, smem_max_size>>>(   \
//         reinterpret_cast<half *>(a.data_ptr()),                                \
//         reinterpret_cast<half *>(b.data_ptr()),                                \
//         reinterpret_cast<half *>(c.data_ptr()), M, N, K);                      \
//   }

// #define LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(stages)\
//   {                                                                            \
//     const int smem_max_size = ((stages) * BM * (BK + A_PAD) * sizeof(half) +   \
//                                (stages) * BK * (BN + B_PAD) * sizeof(half));   \
//     cudaFuncSetAttribute(                                                      \
//         hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_cpbulk_kernel<          \
//             MMA_M, MMA_N, MMA_K, MMA_TILE_M, MMA_TILE_N, WARP_TILE_M,          \
//             WARP_TILE_N, A_PAD, B_PAD, (stages), false, true>,                 \
//         cudaFuncAttributeMaxDynamicSharedMemorySize, 98304);                   \
//     dim3 block(NUM_THREADS);                                                   \
//     dim3 grid(div_ceil(N, BN), div_ceil(M, BM));                               \
//     hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_cpbulk_kernel<              \
//         MMA_M, MMA_N, MMA_K, MMA_TILE_M, MMA_TILE_N, WARP_TILE_M, WARP_TILE_N, \
//         A_PAD, B_PAD, (stages), false, true><<<grid, block, smem_max_size>>>(  \
//         reinterpret_cast<half *>(a.data_ptr()),                                \
//         reinterpret_cast<half *>(b.data_ptr()),                                \
//         reinterpret_cast<half *>(c.data_ptr()), M, N, K);                      \
//   }
...
// 128x128, mma2x4, warp4x4(64,32,16), stages, block swizzle, dsmem
void hgemm_mma_m16n8k16_mma2x4_warp4x4_stages_dsmem_cpbulk(torch::Tensor a,
                                                            torch::Tensor b,
                                                            torch::Tensor c, int stages,
                                                            bool swizzle,
                                                            int swizzle_stride) {
  ...
  if (swizzle) {
    ...
    switch (stages) {
    case 2: // ~17KB
      LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(2, swizzle_stride);
      break;
    case 3: // ~26KB
      LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(3, swizzle_stride);
      break;
    case 4: // ~34KB
      LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(4, swizzle_stride);
      break;
    case 5: // ~43KB
      LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(5, swizzle_stride);
      break;
    default:
      LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(2, swizzle_stride);
      break;
    ...
    switch (stages) {
    case 2:
      LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(2);
      break;
    case 3:
      LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(3);
      break;
    case 4:
      LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(4);
      break;
    case 5:
      LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(5);
      break;
    default:
      LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4_DSMEM_CPBULK_KERNEL(2);
      break;
