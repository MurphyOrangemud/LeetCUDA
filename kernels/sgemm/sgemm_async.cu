...
__global__ void sgemm_t_8x8_sliced_k_f32x4_bcf_dbuf_async_kernel(float *a, float *b, float *c, const int M, const int N, const int K) {
  ...
  int load_a_smem_m = tid / 2;  // (0,1,2,...,127)
  int load_a_smem_k = (tid % 2 == 0)? 0: 4; // (0,4)
  int load_b_smem_k = tid / 32; // 0~7
  int load_b_smem_n = (tid % 32) * 4;   // (0,4,8,...)
...
#pragma unroll
        for (int tk = 0; tk < BK; tk++) {
            // TODO(zhiyi): Bank conflict analysis here
            FLOAT4(r_comp_a[0]) = FLOAT4(s_a[smem_sel][tk][ty * TM]);   // 0,8,16,...,120
            FLOAT4(r_comp_a[4]) = FLOAT4(s_a[smem_sel][tk][ty * TM + 4]);   // 4,12,...,124
            FLOAT4(r_comp_b[0]) = FLOAT4(s_b[smem_sel][tk][tx * TN]);
...
template <const int BM = 128,
    ...
    const int TN = 8,
    const int OFFSET = 0>
__global__ void sgemm_t_8x8_sliced_k16_f32x4_bcf_dbuf_pipeline_kernel(
    float* a, float* b, float* c, const int M, const int N, const int K) {
    // block(BN/TN, BM/TM) -> (x=16,y=16), 256 threads
    const int bx = blockIdx.x;
    const int by = blockIdx.y;
    ...
    __shared__ float s_a[2][BM][BK + OFFSET];
    __shared__ float s_b[2][BK][BN + OFFSET];
    ...
    int load_a_smem_k = (tid % 2 == 0) ? 0 : 8; // (0,8)
    int load_b_smem_k = tid / 16;               // 0~15
    int load_b_smem_n = (tid % 16) * 8;         // (0,8,16,...,120)
    int load_a_gmem_m = by * BM + load_a_smem_m;
    int load_b_gmem_n = bx * BN + load_b_smem_n;

    {
        ...
        int load_b_gmem_addr = load_b_gmem_k * N + load_b_gmem_n;
        uint32_t load_b_smem_ptr = __cvta_generic_to_shared(&s_b[0][load_b_smem_k][load_b_smem_n]);
        uint32_t load_a_smem_ptr = __cvta_generic_to_shared(&s_a[0][load_a_smem_m][load_a_smem_k]);

// 2 cp.async issue, 16 bytes = 4 float.
// #pragma unroll
//         for (int i = 0; i < 8; i += 4) {
//             CP_ASYNC_CG(load_b_smem_ptr + i * 4, &b[load_b_gmem_addr + i], 16);
//         }
// #pragma unroll
//         for (int i = 0; i < 8; i += 4) {
//             CP_ASYNC_CG(load_a_smem_ptr + i * 4, &a[load_a_gmem_addr + i], 16);
//         }
        CP_ASYNC_COMMIT_GROUP();
    }
...
// #pragma unroll
//         for (int i = 0; i < 8; i += 4) {
//             CP_ASYNC_CA(load_a_smem_ptr + i * 4, &a[load_a_gmem_addr + i], 16);
//         }
        CP_ASYNC_COMMIT_GROUP();
...
#pragma unroll
        for (int tk = 0; tk < BK; tk++) {
            FLOAT4(r_comp_a[0]) = FLOAT4(s_a[smem_sel][ty * TM / 2][tk]);
            FLOAT4(r_comp_a[4]) = FLOAT4(s_a[smem_sel][ty * TM / 2 + BM / 2][tk]);
            FLOAT4(r_comp_b[0]) = FLOAT4(s_b[smem_sel][tk][tx * TN / 2]);
            FLOAT4(r_comp_b[4]) = FLOAT4(s_b[smem_sel][tk][tx * TN / 2 + BN / 2]);
...
#pragma unroll
    for (int tk = 0; tk < BK; tk++) {
        FLOAT4(r_comp_a[0]) = FLOAT4(s_a[1][ty * TM / 2][tk]);
        FLOAT4(r_comp_a[4]) = FLOAT4(s_a[1][ty * TM / 2 + BM / 2][tk]);
        FLOAT4(r_comp_b[0]) = FLOAT4(s_b[1][tk][tx * TN / 2]);
        FLOAT4(r_comp_b[4]) = FLOAT4(s_b[1][tk][tx * TN / 2 + BN / 2]);
